...
contract UnichainFrens is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function updateGlobalRewards(uint256 fee, bool isNewMoney) private {
        if (totalWeightPoints > 0) {
            // Calculate total pending rewards before update
            uint256 currentPendingRewards = 0;
            uint256 supply = totalSupply();
            for (uint256 i = 1; i <= supply; i++) {
                if (positionWeightPoints[i] > 0) {
                    currentPendingRewards += getPendingRewards(i);
                }
            }
            
            // Calculate how much we can safely distribute
            uint256 availableBalance = address(this).balance;
            require(availableBalance >= currentPendingRewards, "Contract underwater");
            
            uint256 safeToDistribute = availableBalance - currentPendingRewards;
            if (fee > safeToDistribute) {
                fee = safeToDistribute;
            }
            
            // Only proceed if we have something to distribute
            if (fee > 0) {
                uint256 adjustedTotalWeightPoints = totalWeightPoints;
                
                // Calculate rewards per weight point based on the safe fee amount
        ...
        // Only add to totalRewards if this is new money and we actually distributed it
        if (isNewMoney && fee > 0) {
            totalRewards += fee;
        }
    }
    ...
    function getPendingRewards(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 supply = totalSupply();
        for (uint256 i = 1; i <= supply; i++) {
            if (i != tokenId && positionWeightPoints[i] > 0) {
                uint256 otherRewards = (positionWeightPoints[i] * 
                    (rewardsPerWeightPoint - lastRewardsPerWeightPoint[i])) / 1e18;
                otherRewards = (otherRewards / 1e2) * 1e2;
                totalPending += otherRewards;
            }
        }
        
        // Cap rewards based on available balance after accounting for other positions
        uint256 availableBalance = address(this).balance;
        if (totalPending >= availableBalance) return 0;
        
        uint256 safeMaxReward = availableBalance - totalPending;
        if (rewardsAccrued > safeMaxReward) {
            rewardsAccrued = safeMaxReward;
        }
    ...
    function redistribute(uint256 tokenId) external nonReentrant returns (uint256 newWeight) {
        ...
        // Calculate total pending rewards excluding this position
        uint256 otherPendingRewards = 0;
        uint256 supply = totalSupply();
        for (uint256 i = 1; i <= supply; i++) {
            if (i != tokenId && positionWeightPoints[i] > 0) {
                otherPendingRewards += getPendingRewards(i);
            }
        }
        
        // Verify contract can handle this redistribution
        uint256 currentBalance = address(this).balance;
        require(currentBalance >= otherPendingRewards, "Insufficient contract balance");
        ...
        // Only redistribute if we have other positions and it's safe to do so
        if (redistributeAmount > 0 && totalSupply() > 1) {
            // Calculate other positions' total weight points
            uint256 otherWeightPoints = 0;
            for (uint256 i = 1; i <= supply; i++) {
                if (i != tokenId && positionWeightPoints[i] > 0) {
                    otherWeightPoints += positionWeightPoints[i];
                }
            }
            
            // Only proceed if there are other positions to redistribute to
            if (otherWeightPoints > 0) {
                // Only redistribute what's safely available
                if (currentBalance > otherPendingRewards) {
                    uint256 safeToRedistribute = currentBalance - otherPendingRewards;
                    if (redistributeAmount > safeToRedistribute) {
                        redistributeAmount = safeToRedistribute;
                    }
                    
                    if (redistributeAmount > 0) {
                        // Temporarily set totalWeightPoints to just other positions
                        uint256 originalTotalWeightPoints = totalWeightPoints;
                        totalWeightPoints = otherWeightPoints;
                        
                        // Update global rewards (this will only affect other positions)
                        updateGlobalRewards(redistributeAmount, false);
                        
                        // Restore total weight points (still excluding our old points)
                        totalWeightPoints = originalTotalWeightPoints;
                    }
                }
            }
        }
        
        // Update position state after redistribution
    ...
    function softWithdraw(uint256 tokenId) external nonReentrant returns (uint256 newWeight) {
        ...
        // Verify contract can handle this operation
        uint256 currentBalance = address(this).balance;
        require(currentBalance >= otherPendingRewards + withdrawAmount, "Insufficient contract balance");
        ...
        if (redistributeAmount > 0 && totalSupply() > 1) {
            ...
            if (otherWeightPoints > 0) {
                // Only redistribute what's safely available
                if (currentBalance > otherPendingRewards + withdrawAmount) {
                    uint256 safeToRedistribute = currentBalance - (otherPendingRewards + withdrawAmount);
                    if (redistributeAmount > safeToRedistribute) {
                        redistributeAmount = safeToRedistribute;
                    }
    ...
    function hardWithdraw(uint256 tokenId) external nonReentrant {
        ...
        // Calculate inverted weight points (favoring later positions)
        uint256 otherWeightPoints = 0;
        uint256 invertedWeightPoints = 0;
        for (uint256 i = 1; i <= supply; i++) {
            if (i != tokenId && positionWeightPoints[i] > 0) {
                // Get the base weight for this position
                uint256 weight = positionWeights[i];
                uint256 basePoints = positionWeightPoints[i];
                otherWeightPoints += basePoints;
                
                // Calculate inverted multiplier: instead of 1/pos^2, use pos^2
                // This makes later positions get more of the redistribution
                uint256 invertedMultiplier = (i * i * 1e18) / (tokenId * tokenId);
                invertedWeightPoints += (weight * invertedMultiplier);
            }
        }
        ...
        claimedRewards[tokenId] += withdrawAmount; // Only count what's actually withdrawn
        lastRewardsPerWeightPoint[tokenId] = rewardsPerWeightPoint;
        
        // Only redistribute if we have other positions and it's safe to do so
        if (redistributeAmount > 0 && totalSupply() > 1 && otherWeightPoints > 0) {
            // Only redistribute what's safely available
            if (currentBalance > otherPendingRewards + withdrawAmount) {
                uint256 safeToRedistribute = currentBalance - (otherPendingRewards + withdrawAmount);
                if (redistributeAmount > safeToRedistribute) {
                ...
                if (redistributeAmount > 0) {
                    // Temporarily set totalWeightPoints to inverted weights that favor later positions
                    totalWeightPoints = invertedWeightPoints;
                    
                    // Update global rewards (this will give more rewards to later positions)
                    updateGlobalRewards(redistributeAmount, false);
                    
                    // Restore total weight points to normal (excluding our zeroed position)
                    totalWeightPoints = otherWeightPoints;
                }
            }
        }
