...
contract Unifrens is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function updateGlobalRewards(uint256 fee, bool isNewMoney) private {
        if (totalWeightPoints > 0) {
            ...
            // First divide to maintain precision, then multiply by 1e18
    ...
    function softWithdraw(uint256 tokenId) external nonReentrant returns (uint256 newWeight) {
        ...
        if (oldWeight < MAX_WEIGHT) {
            // Calculate and update weight increase (about half of what redistribute would give)
        ...
        // Update claimed rewards before event and transfer
    ...
    function _beforeTokenTransfer(
        ...
        // No special handling needed since burn is handled separately
    }
     ...
     * This allows the last active position to claim the entire contract balance, if any
